    ...
    <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-6">
        <div class="mb-4">
            @if($order->status == 'paid')
                ...
                <h2 class="text-2xl font-bold text-green-700">Pembayaran Berhasil!</h2>
                <p class="text-gray-600 mt-2">Terima kasih, pesanan Anda sedang diproses.</p>
            @else
                <div class="text-amber-600 text-6xl mb-2">⏳</div>
                <h2 class="text-2xl font-bold text-amber-700">Menunggu Pembayaran</h2>
                <p class="text-gray-600 mt-2">Silakan selesaikan pembayaran Anda.</p>
            @endif
        </div>

        <div class="bg-gray-100 dark:bg-gray-700 p-4 rounded-xl text-left">
            ...
            <div class="flex justify-between pt-2">
                <span>Status</span>
                <span class="capitalize font-semibold {{ $order->status == 'paid' ? 'text-green-600' : 'text-amber-600' }}">{{ $order->status }}</span>
            </div>
        </div>
        ...
        <a href="{{ route('home') }}" class="block mt-4 text-amber-600 underline">Kembali ke Beranda</a>
    </div>
</div>
@endsection